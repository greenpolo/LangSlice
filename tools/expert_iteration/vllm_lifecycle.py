...
def merge_lora_to_bf16(
    *,
    ...
    """Run the merge script inside the training container.

    Writes the merge driver into ``workdir`` (mounted by docker-compose
    via the bind mount at /workspace/LangSlice), then invokes
    ``docker compose run --rm training`` to execute it. The container has
    ``transformers`` + ``peft`` + ``torch`` installed via the base image.
    """
    ...
    cmd = [
        "docker", "compose", "-f", str(base_compose), "run", "--rm",
        "training", "bash", "-lc",
        (
            "set -euo pipefail && "
