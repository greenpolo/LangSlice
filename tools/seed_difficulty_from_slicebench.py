...
# Make the gemma single-turn RL package + slicebench importable when the
# tool is run from the repo root. ``manifest_index`` lives under
# ``models/langslice-gemma-4/training`` (per pyproject pythonpath) and
# ``slicebench`` lives at the repo root.
_REPO_ROOT = Path(__file__).resolve().parent.parent
_GEMMA4_TRAINING = _REPO_ROOT / "models" / "langslice-gemma-4" / "training"
        ...
        rows.append(
            {
                ...
                "difficulty_score": bin_mae / extent,
                "source": source_label,
            }
        )
