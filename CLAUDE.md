...
- **`langslice/brain/`** — Whole-brain multi-slice AP estimation. Anchors are estimated with 3-pass nano-banana (full atlas range), then all remaining slices are estimated in parallel with 2-pass nano-banana (windowed). A Huber-loss constrained optimizer fits a monotone curve through all estimates, with local-interval spacing priors and hard minimum-thickness constraints. CLAHE adaptive preprocessing is applied to all slices. Public entry point: `run_brain_estimation(...)` in `pipeline.py`.
