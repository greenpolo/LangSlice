...
Sibling of ``langslice_training.embeddings.precompute`` (which caches atlas reference images);
...
2. ``processor.apply_chat_template`` (image-only message) — same processor
   path as ``langslice_training.embeddings.precompute._encode_pil``, so the SigLIP
...
def _encode_pil(model: Any, processor: Any, pil_image, *, device: str, dtype: torch.dtype):
    ...
    Identical to :func:`langslice_training.embeddings.precompute._encode_pil` so the cached
...
def precompute_query_embeddings(
    ...
    *,
    ...
    from langslice_training.embeddings.query_cache import save_query_pair
