# Dynamic Difficulty Curriculum (LIVE, flag-gated)

> Lifted from `training/curriculum/` to `training/adaptive/curriculum/` on
> 2026-05-11 as part of Phase 6 of the iSFT speed upgrade. The old path
> still resolves via a `__getattr__` shim in `training/curriculum/__init__.py`,
> but new code should import from `adaptive.curriculum` directly.
...
Defaults to uniform weights when no `--curriculum-weights-dir` is passed —
behavior is byte-identical to the no-curriculum path in that case.

For the surrounding adaptive package see [`../README.md`](../README.md);
...
| `cli.py` | Standalone `python -m adaptive.curriculum.cli` weight-update tool (legacy `python -m curriculum.cli` still works via shim). |
...
For single-turn RL, add to `langslice-single-turn-rl ...`:
```
