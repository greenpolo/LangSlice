...
Usage::
    ...
    python -m embeddings.render_slates_from_cache \\
...
# isn't a normal Python package (the dir name has a hyphen). This script lives
# under models/langslice-gemma-4/training/embeddings/, so the training root is
...
def _render_one_cache(cache_path: Path, output_root: Path, *, overwrite: bool, quality: int) -> tuple[int, int, int]:
    """Render every JPG named in one .pt cache. Returns (rendered, skipped_existing, skipped_bad_key)."""
    ...
    for i, key in enumerate(keys):
        ...
        try:
            ...
            logger.warning("render failed for %s/%s @ %.2fmm: %s", atlas_name, plane, position_mm, exc)
            skipped_bad_key += 1
            continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not embedding_cache_dir.is_dir():
        logger.error("--embedding-cache %s does not exist or is not a directory", embedding_cache_dir)
        return 2
    ...
    for i, cache_path in enumerate(cache_files):
        ...
        try:
            r, se, sb = _render_one_cache(cache_path, output_root, overwrite=args.overwrite, quality=args.quality)
        except Exception as exc:
