"""Phase 5 Task 5.2 smoke test: observe zoom-tool invocations in a group run.
...
- Soft: at least one ``zoom`` tool call observed during the run. If the soft
  gate fails the script returns exit 2 so the caller can decide whether to
  invest in prompt tweaks — we do not iterate on prompts here.
"""
...
async def run_smoke() -> dict:
    ...
    # Seed each target image as 'target:N' so the zoom tool can reference them.
...
def main() -> int:
    ...
    try:
        ...
        (out_dir / "post_step5_zoom_M01.json").write_text(
    ...
    zoom_calls = tool_counts.get("zoom", 0)
    gate_pass_hard = n_errors == 0 and not fallback and not capped
    gate_pass_soft = zoom_calls >= 1
    ...
    out_dir = REPO_ROOT / "eval_outputs"
    out_dir.mkdir(exist_ok=True)
    out_path = out_dir / "post_step5_zoom_M01.json"
    ...
    if not gate_pass_soft:
        print(
            "WARN: zoom invocations == 0; soft gate failed. "
